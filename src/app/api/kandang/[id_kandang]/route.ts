...
export async function PUT(req: Request, { params }: { params: { id_kandang: string } }) {
  ...
  try {
    ...
    if (existingKandang.gambar_kandang !== null) {
      ...
      const existingImagePath = join(process.cwd(), "public", existingKandang.gambar_kandang);
    ...
    if (file !== null) {
      ...
      const newImagePath = join(process.cwd(), "public", "images", "kandang", namaFile);

      await writeFile(newImagePath, buffer);

      updatedKandangData.gambar_kandang = `/images/kandang/${namaFile}`;
    }
...
// const { nama_kandang } = body;

// const { kandangId } = params;
...
//   try {
//     const id_kandang = req.url.split("/kandang/")[1];

//     const data = await req.formData();

//     const nama_kandang: string | null = data.get("nama_kandang") as unknown as string | null;

//     const post = await db.kandang.update({
//       where: { id_kandang: id_kandang },
//       data: {
//         nama_kandang: nama_kandang,
//       },
...
//     return NextResponse.json({ kandang: post, message: "Data kandang berhasil diperbarui" }, { status: 200 });
//   } catch (error) {
//     return NextResponse.json({ message: "Terjadi kesalahan dalam memperbarui data kandang." }, { status: 500 });
//   }
// };
